use log::{error, info};
use rocksdb::{AsColumnFamilyRef, ColumnFamily, ColumnFamilyDescriptor, IteratorMode, Options, DB};
...
impl Database {
    pub fn open<P: AsRef<Path>>(path: P, cfs: &[String]) -> Self {
    ...
    pub fn put_cf<T>(&self, cf: &impl AsColumnFamilyRef, key: &str, value: &T) -> Result<(), String>
    where
    ...
    pub fn get_cf<T: DeserializeOwned>(
        ...
        match self.0.get_cf(cf, key) {
            Ok(value) => match value {
                Some(bytes) => Self::deserialize_from_bytes(bytes),
                None => Ok(None),
            },
    ...
    pub fn last_items_in_cfs<'a, T: DeserializeOwned>(
        &self,
        cfs: &'a [String],
        ...
        cfs.iter()
            .map(|cf| match self.0.cf_handle(cf) {
                Some(cf_handle) => match self.0.iterator_cf(cf_handle, IteratorMode::End).next() {
                    Some((_, value)) => match Self::deserialize_from_bytes(value.to_vec()) {
    ...
    fn deserialize_from_bytes<T: DeserializeOwned>(bytes: Vec<u8>) -> Result<Option<T>, String> {
...
mod tests {
    ...
    fn test_last_items_in_cfs() {
        ...
        let items_map: HashMap<&str, Option<String>> = db.last_items_in_cfs(&cfs);
